...
public class HeapSort {
     ...
     * 2. 维护堆的性质(只需将根节点与其左右子节点比较即可，若有替换再维护子节点堆的性质)
    ...
    public static void sort(int[] arr) {
        ...
        //1. 建堆，从最后一个非叶子节点开始 arr.length / 2 - 1，往前维护每个子树(堆的性质)
        for (int i = arr.length / 2 - 1; i >= 0; i--)
            heapify(arr, arr.length, i);


        //2.堆排序，第一次堆排序时，最后一位已有序，也就是维护堆得性长度从arr.length - 1开始，也就是
